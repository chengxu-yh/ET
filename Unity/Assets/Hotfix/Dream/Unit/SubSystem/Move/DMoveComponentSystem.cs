...
namespace ET
{
    ...
    {
        public static bool IsArrived(this DMoveComponent self)
        {
            return self.Targets.Count == 0;
        }

        public static bool ChangeSpeed(this DMoveComponent self, float speed)
        {
            if (self.IsArrived())
            {
                return false;
        ...
        public static async ETTask<bool> MoveToAsync(this DMoveComponent self, List<Vector3> target, float speed, int turnTime = 100, ETCancellationToken cancellationToken = null)
        {
            self.Stop();
            ...
            Game.EventSystem.Publish(new AppEventType.MoveStart(){Unit = self.GetParent<DUnit>()}).Coroutine();
            ...
            if (moveRet)
            {
                Game.EventSystem.Publish(new AppEventType.MoveStop(){Unit = self.GetParent<DUnit>()}).Coroutine();
            }
            return moveRet;
        }

        public static void MoveForward(this DMoveComponent self, bool needCancel)
        {
            DUnit unit = self.GetParent<DUnit>();
        ...
        private static void StartMove(this DMoveComponent self)
        {
            DUnit unit = self.GetParent<DUnit>();
        ...
        private static void SetNextTarget(this DMoveComponent self)
        {
        ...
        private static Vector3 GetFaceV(this DMoveComponent self)
        {
            return self.NextTarget - self.PreTarget;
        }

        public static bool FlashTo(this DMoveComponent self, Vector3 target)
        {
            DUnit unit = self.GetParent<DUnit>();
        ...
        public static bool MoveTo(this DMoveComponent self, Vector3 target, float speed, int turnTime = 0, bool isTurnHorizontal = false)
        {
            if (speed < 0.001)
            {
                Log.Error($"speed is 0 {self.GetParent<DUnit>().Id} {speed}");
                return false;
            }
            ...
            self.TurnTime = turnTime;
            self.Targets.Add(self.GetParent<DUnit>().Position);
        ...
        public static bool MoveTo(this DMoveComponent self, List<Vector3> target, float speed, int turnTime = 0)
        {
            if (target.Count == 0)
            {
                return true;
            ...
            {
                Log.Error($"speed is 0 {self.GetParent<DUnit>().Id}");
                return false;
            }
        ...
        public static void Stop(this DMoveComponent self)
        {
        ...
        public static void Clear(this DMoveComponent self)
        {
            self.StartTime = 0;
            self.StartPos = Vector3.zero;
            self.BeginTime = 0;
